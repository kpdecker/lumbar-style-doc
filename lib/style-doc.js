...
module.exports = function(input, options) {
  ...
  for (var i = 0, len = ast.length; i < len; i++) {
    ...
    if (!/^hidden_/.test(token.lang)) {
      updated.push(token);
    } else {
      token.lang = token.lang.substring('hidden_'.length);
    }
    ...
    if (token.type === 'code') {
      ...
      if (token.lang === 'html') {
        updated.push({
          type: 'html',
          pre: true,
          text: '<div id="style-doc-' + (++id) + '" class="style-doc-sample">' + token.text + '</div>'
        });
      } else if (token.lang === 'javascript') {
        updated.push({
          type: 'html',
          pre: true,
          text: '<script type="style-doc">' + token.text + '</script>'
        });
        ...
        updated.push({
          type: 'html',
          pre: true,
          text: '<div id="style-doc-' + uniqueId + '" class="style-doc-sample"></div>\n'
            + '<script type="style-doc">document.getElementById("style-doc-' + uniqueId + '").innerHTML = '
              + 'Handlebars.template(' + Handlebars.precompile(token.text) + ')(context || {});</script>'
        });
        ...
        var compiler = stylus(token.text);
        _.each(options.stylusIncludes, function(include) {
          compiler.import(include.src || include);
        });
        compiler.render(function(err, style){
          if (err) {
            throw err;
          }
      ...
      } else if (token.lang === 'taphighlight') {
        updated.pop();

        updated.push({
          type: 'html',
          pre: true,
          text: '<script type="style-doc">highlightStyles(' + id + ', "' + escapeJavascript(token.text) + '");</script>'
        });
      }
    }
  }
  ...
  if (options && options.templates) {
    _.each(options.templates, function(value, key) {
      templates[escapeJavascript(key)] = Handlebars.precompile(value);
    });
  }
